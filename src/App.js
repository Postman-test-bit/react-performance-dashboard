...
function App() {
  ...
  const availableURIs = useMemo(
    () =>
      [...new Set(allData.map((d) => d.url || d.uri || ""))]
        .filter(Boolean)
        .sort(),
    [allData]
  );
  ...
  const availableProducts = useMemo(
    () =>
      [...new Set(allData.map((d) => d.brand || d.product || ""))]
        .filter(Boolean)
        .sort(),
    [allData]
  );
  ...
  const filteredData = useMemo(() => {
    return allData.filter((item) => {
      ...
      const itemDevices = Array.isArray(item.tags)
        ...
        ? [item.device]
        : [];
      const matchesDevice = !deviceFilter || itemDevices.includes(deviceFilter);
          ...
          data.reduce((acc, curr) => {
            if (
              ...
              new Date(curr.created_at) >
                new Date(acc[curr.scenario].created_at)
            ) {
