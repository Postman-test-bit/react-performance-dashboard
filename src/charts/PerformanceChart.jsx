...
const PerformanceChart = ({ data, theme = "light" }) => {
    ...
    return Object.entries(grouped)
      .map(([scenario, entries]) => {
        ...
        const scores = sorted.map(
          (item) => parseFloat(item.performance_metrics) || 0
        );
        const currentScore = scores[scores.length - 1]; // Latest score
        const previousScore =
          scores[scores.length - 2] || scores[scores.length - 1]; // Previous score or latest if not available
  ...
  const sparklineOptions = (scenarioData) => ({
    ...
    yAxis: {
      ...
      min:
        Math.min(
          ...scenarioData.entries.map((e) => parseFloat(e.performance_metrics))
        ) - 5,
      max:
        Math.max(
          ...scenarioData.entries.map((e) => parseFloat(e.performance_metrics))
    ...
    tooltip: {
      ...
      formatter: function () {
        // Custom tooltip showing all three data points
    ...
    series: [
      {
        data: scenarioData.entries.map((e) =>
          parseFloat(e.performance_metrics)
        ),
        color:
        ...
        >
          ...
          <div
            style={{
              display: "grid",
              gridTemplateColumns:
                window.innerWidth < 768 ? "1fr 1fr" : "auto auto auto auto",
              gap: "4px",
            }}
          >
