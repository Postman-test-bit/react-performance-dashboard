// src/charts/BestPracticeChart.jsx
...
const BestPracticeChart = ({ data, allData, theme }) => {
  ...
  const [bestPracticeFilter, setBestPracticeFilter] = useState("All");

  const deviceTypes = [...new Set(allData.map((d) => d.device))];

  useEffect(() => {
    applyFilters();
  }, [deviceFilter, bestPracticeFilter, data]);
  ...
  const applyFilters = () => {
    ...
    if (bestPracticeFilter !== "All") {
      ...
      filtered = filtered.filter(
        (d) => d.best_practice_metrics >= min && d.best_practice_metrics <= max
      );
    }
  ...
  const barData = testNames.map((test) => {
    const score = latestTests[test][0].best_practice_metrics;
    return {
      name: test,
      score,
      fill:
  ...
  testNames.forEach((test) => {
    tooltipData[test] = latestTestsTooltip[test]
      ...
      .map((run) => ({
        ...
        score: run.best_practice_metrics,
      }));
  });
  ...
  return (
    <ChartContainer title="Best-Practice for Last 3 Runs" filters={filters}>
      <div
